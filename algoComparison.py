...
class AlgoComparison(BaseEstimator):
    ...
    def CV(self, X, y, loss_weights=None, shuffle = False):
        ...
        def cross_val_with_learning_curves(self, X, y, scoring = 'neg_mean_absolute_error', n_jobs = 1,
            ...
            def errorAnalysis(self, X, y, dataTable, bcm_pe_table, dataSetType, rank_by='MAE', sample_weight=None, percentile=None,
                              no_of_plots=30, normalize_by=None, save=True):
                # Plots no of plots number deals around different percentiles of error, e.g. if percentile = 0.5, and no_of_plots=30, find the index of 50th percentile deal and plots the 15 deals below it and the 15 deals above it
                # Percentile = 'top', 'bottom', 'random', or numerical value between 0 and 1
                # percentile = 30, plots no_of_plots number of random deals
                ...
                for model, name in self.models.items():
                    ...
                    df['squared_error'] = ((df['Bcm_pe'] - df['Y_pred'])**2) * df['sample_weights']
                ...
                # deals are on the index of the df_per_deal table. It grabs the index as lists and slices that list
